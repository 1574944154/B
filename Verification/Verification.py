...
class CrackGeetest():
    ...
    def verify(self):
        try:
            ...
            bg_location_list, fullgb_location_list = self.get_geetest_image()
            img1 = self.get_merge_image('Image/fullgb.jpg', fullgb_location_list)
            img2 = self.get_merge_image('Image/bg.jpg', bg_location_list)
            # distance应根据实际情况做微调
            distance = self.get_gap(img1, img2) * 1.138
...
if __name__ == '__main__':
    ...
    browser.get("http://account.bilibili.com/answer/base/#/")
    browser.find_element_by_id("login-username").send_keys("19923393852")
    browser.find_element_by_id("login-passwd").send_keys("a510b630")
    CrackGeetest(browser).verify()
